...
const DetailedStatsTable = ({ player }: StatsTableProps) => {
	...
	const filteredStats = yearByYearStats?.splits.filter((elem) => {
		...
		if (currentLeauge !== 'all') return elem.league?.name === currentLeauge ? elem : null
	...
	return (
		<div className='w-[99%] mx-auto'>
			...
			/>
			<h1 className='text-2xl mt-3'>{`${playerData.fullName} Career Stats`}</h1>
			<div className='overflow-x-auto rounded-md w-full mx-auto'>
			{filteredStats.length > 0 ? (

<table className='table text-sm w-full mt-2 rounded-md text-center p-4 border-collapse overflow-x-auto '>
	...
	{filteredStats.map((stat) => {
		return (
			<tr className='table-row' key={uuidv4()}>
				...
				<td className=' px-[8px] py-[13px]'>{stat.stat?.assists}</td>
				<td className=' px-[8px] py-[13px]'>{stat.stat?.points}</td>
				<td className=' px-[8px] py-[13px]'>{stat.stat?.plusMinus || '--'}</td>
				<td className=' px-[8px] py-[13px]'>{stat.stat?.penaltyMinutes || '--'}</td>
				<td className=' px-[8px] py-[13px]'>{stat.stat?.powerPlayGoals || '--'}</td>
				<td className=' px-[8px] py-[13px]'>{stat.stat?.powerPlayPoints || '--'}</td>
				<td className=' px-[8px] py-[13px]'>{stat.stat?.shortHandedGoals || '--'}</td>
				<td className=' px-[8px] py-[13px]'>{stat.stat?.shortHandedPoints || '--'}</td>
				<td className=' px-[8px] py-[13px]'>{stat.stat?.gameWinningGoals || '--'}</td>
				<td className=' px-[8px] py-[13px]'>{stat.stat?.overTimeGoals || '--'}</td>
				<td className=' px-[8px] py-[13px]'>{stat.stat?.shots || '--'}</td>
				<td className=' px-[8px] py-[13px]'>{stat.stat?.shotPct || '--'}</td>
				<td className=' px-[8px] py-[13px]'>{stat.stat?.faceOffPct || '--'}</td>
			</tr>
		);
	})}
	{reducedStats && currentLeauge !== 'all' && (
		<tr>
			<td className=' px-[8px] py-[13px] sticky left-0 bg-black'>Career</td>
			<td className=' px-[8px] py-[13px]'>{'-'}</td>
			<td className=' px-[8px] py-[13px]'>{reducedStats.stat?.games}</td>
			<td className=' px-[8px] py-[13px]'>{reducedStats.stat?.goals}</td>
			<td className=' px-[8px] py-[13px]'>{reducedStats.stat?.assists}</td>
			<td className=' px-[8px] py-[13px]'>{reducedStats.stat?.points}</td>
			<td className=' px-[8px] py-[13px]'>{reducedStats.stat?.plusMinus || '--'}</td>
			<td className=' px-[8px] py-[13px]'>{reducedStats.stat?.pim || '--'}</td>
			...
			<td className=' px-[8px] py-[13px]'>{reducedStats.stat?.shots || '--'}</td>
			<td className=' px-[8px] py-[13px]'>
